...
# beacon ages by about one poll across its terminal wait, so a fixed 300s grace
# stops correctly bounding staleness once the poll cadence reaches or exceeds
# it; growing the default with the cadence while keeping the historical 300s
# floor for the common short-poll case fixes that without a caller-specific
# constant.
