...
# fm-watch.sh touches the liveness beacon at the top of every cycle and keeps
# it fresh through a long cycle, so a healthy watcher's beacon can legitimately
# age by about FM_POLL seconds across its terminal wait (docs/turnend-guard.md
# "Guard grace and the poll cadence"). fm_poll_derived_grace (bin/fm-wake-lib.sh) is the single
