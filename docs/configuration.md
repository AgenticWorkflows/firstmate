...
FM_SPAWN_WORKTREE_WAIT_SECS=60           # seconds fm-spawn.sh waits for `treehouse get` to settle the new pane in an isolated pool copy before refusing the spawn; raise it on a loaded host whose pool is slow to hand a copy over; a value that is not a positive integer falls back to the default
